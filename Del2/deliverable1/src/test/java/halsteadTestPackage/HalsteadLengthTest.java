...
import halsteadPackage.HalsteadLength;
...
public class HalsteadLengthTest {
	...
	@Test
	void beginTreeTest() {
		 HalsteadLength spyLength = spy(new HalsteadLength());
	...
	void getDefaultTokensTest() {
		...
		assertArrayEquals(tokenTypes, length.getDefaultTokens());
	}
	...
	@Test
	void getAcceptableTokenTest() {
		HalsteadLength length = new HalsteadLength();
		assertArrayEquals(tokenTypes, length.getAcceptableTokens());
	}
	
	@Test
	void getRequiredTokenTest() {
		HalsteadLength length = new HalsteadLength();
		assertArrayEquals(tokenTypes, length.getRequiredTokens());
	}
	...
	void finishTreeTest() {
		HalsteadLength spyLength = spy(new HalsteadLength());
		DetailAST mockAST = mock(DetailAST.class);
		String message = "Halstead symbols detected within a specific class: 0";
